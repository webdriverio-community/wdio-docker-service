...
import type { DockerLauncherConfig } from '@root/launcher.js'
import { DockerLauncherForTests as DockerLauncher } from '@root/launcher.js'
...
vi.mock('@root/utils/docker.js', () => {
    class Docker {
        ...
        constructor(image: string, { options = {}, healthCheck = {}, command, args, debug = false }: { options?: Record<string, unknown>, healthCheck?: string | { url: string } | Record<string, never>, command?: string, args?: string, debug?: boolean } = {}) {
            this.image = image
            this.args = args
        ...
        once(event: string, cb: (...args: unknown[]) => void) { this.listeners[event] = cb }
    ...
    Docker.prototype.once = vi.fn().mockImplementation(function(this: Docker, event: string, cb: (...args: unknown[]) => void) {
